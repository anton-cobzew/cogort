...
<body>
	<?php
		require('template/head_menu.php');
	?>
		...
		<?php
			...
			foreach ($friends_ids as $id) {
				$f = mysqli_fetch_assoc(send_sql($conn,
								   "SELECT nickname, name, status, about
								   FROM users
								   WHERE id = " . $id));

				$avatar_file = 'img/avatar/' . $f['nickname'] . '.jpg';
				if (!file_exists($avatar_file)) {
					$avatar_file = 'img/avatar/' . $f['nickname'] . '.png';
					if (!file_exists($avatar_file))
						$avatar_file = 'img/avatar/default.jpg';
				}

				echo '<li class="friends_list__item">
